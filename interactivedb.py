...
def insert_work():
    ...
    published_at = input("Published date (YYYY-MM-DD, optional): ").strip() or None
    rating = input("Rating (General Audiences, Teen, Mature, Explicit, default General Audiences): ").strip() or "General Audiences"

    # Insert work
    try:
        cursor.execute("""
            INSERT INTO works (title, author, fandom, summary, notes, word_count, chapters, complete, published_at, rating)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
        """, (title, author, fandom, summary, notes, word_count, chapters, complete, published_at, rating))
        db.commit()
